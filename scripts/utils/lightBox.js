...
async function displayLightBox(idMedias,photographerId) {
	...
	if (response.ok) {
		...
		const MediaToPhotograp = data.media.filter((medias) => medias.photographerId === photographerId);
		...
		{	//Si le media corssepond au media actuellement affiché
			if(MediaToPhotograp[i].id==idMedias)
			{	//Si on est au dernier media on affecte au media suivant le media actuelle
